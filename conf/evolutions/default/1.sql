...
create table tokens (
  ...
  created                   datetime,
  expires                   datetime,
  constraint ck_tokens_type check (type in (0,1)),
...
alter table tokens add constraint fk_tokens_targetUser_2 foreign key (target_user_id) references users (id) on delete restrict on update restrict;
create index ix_tokens_targetUser_2 on tokens (target_user_id);
